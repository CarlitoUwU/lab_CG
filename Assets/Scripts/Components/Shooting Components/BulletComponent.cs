        ...
        if (Physics.Raycast(transform.position, move, out hit, moveDist, ~NO_BULLET_COLLIDE_LAYER, QueryTriggerInteraction.Ignore))
        {
            ...
            if (!hitSelf)
            {
                ...
                if (damageable != null)
                {
                    ...
                    // Si no está en el collider directo, buscar en hijos del damageable
                    if (headshot == null)
                    {
                        ...
                        foreach (HeadshotComponent hs in headshotsInEnemy)
                        {
                            ...
                            {
                                closestDist = dist;
                                headshot = hs; // ← AQUÍ estaba el bug: nunca se asignaba
                            }
                        }

                        // Umbral: si el impacto está a más de 1 unidad de la cabeza, no es headshot
                        if (closestDist > 1.0f)
                            headshot = null;
                    }
